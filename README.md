...
**项目简介**：面向金庸武侠读者，构建带有**时代维度与门派-武功-人物层次结构**的武侠知识图谱，支持**跨代际关系追溯、武功传承推理与交互式关系探索**。系统根据问题类型触发领域专用检索策略，动态选择最优知识注入形式，并在交互式图谱中可视化推理证据。
...
本项目面向金庸武侠这一“强门派归属、强武功传承、强代际关系”的领域，研究**层次化知识结构如何支持武侠多跳推理**：

1. **层次化 KG 的有效性**：引入“门派 → 人物 → 武功 → 时代”的层次结构后，相比扁平三元组，能否提升“郭靖的师父的师父是谁”这类**跨代际多跳问题**的推理准确率？
2. **领域检索策略的鲁棒性**：师徒链、门派聚合等**领域专用检索策略**，相比通用图检索，能否过滤**跨书噪声**（如把《倚天》人物错连到《射雕》）？
3. **知识注入形式的适配性**：不同的知识注入形式（扁平三元组 / 关系路径 / 子图摘要）对 LLM 武侠领域推理的能力有何差异？

这三个问题分别对应 GraphRAG 的**知识构建（层次化）**、**检索（领域策略）**、**知识注入**三大环节，核心是“武侠领域知识如何帮助推理”，而非通用检索算法的横向对比——这正是本项目区别于通用方法实验平台（如影视领域基座）的差异化所在。
