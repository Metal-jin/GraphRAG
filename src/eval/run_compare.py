...
运行示例（在仓库根目录）：
    python -m src.eval.run_compare --methods vector library_graphrag pathrag tcpr
...
# README 中约定的四个方法名。用户可以通过 --methods 只跑其中几个。
DEFAULT_METHODS = ("vector", "library_graphrag", "pathrag", "tcpr")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--methods", nargs="+", default=list(DEFAULT_METHODS), help="要运行的方法名，默认运行四种方法")
